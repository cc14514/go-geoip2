...
type TestType struct {
	Array      []uint                 `driver:"array"`
	Boolean    bool                   `driver:"boolean"`
	Bytes      []byte                 `driver:"bytes"`
	Double     float64                `driver:"double"`
	Float      float32                `driver:"float"`
	Int32      int32                  `driver:"int32"`
	Map        map[string]interface{} `driver:"map"`
	Uint16     uint16                 `driver:"uint16"`
	Uint32     uint32                 `driver:"uint32"`
	Uint64     uint64                 `driver:"uint64"`
	Uint128    big.Int                `driver:"uint128"`
	Utf8String string                 `driver:"utf8_string"`
}
...
func TestNonEmptyNilInterface(t *testing.T) {
	...
	assert.Equal(t, err.Error(), "driver: cannot unmarshal map into type driver.TestInterface")
}

type CityTraits struct {
	AutonomousSystemNumber uint `json:"autonomous_system_number,omitempty" driver:"autonomous_system_number"`
}

type City struct {
	Traits CityTraits `driver:"traits"`
}
...
type ValueTypeTestType struct {
	Boolean BoolInterface `driver:"boolean"`
}
...
type NestedMapX struct {
	UTF8StringX string `driver:"utf8_stringX"`
}

type NestedPointerMapX struct {
	ArrayX []int `driver:"arrayX"`
}
